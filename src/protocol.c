...
int receive_message(const int msgtype, ...) {
    ...
    if (msgtype == RESPONSE) {
        Msg_response * msg = va_arg(vl, Msg_response *);
        i = msgrcv(own_id, msg, sizeof(int) + RESPONSE_LENGTH, msgtype, IPC_NOWAIT);
    }
    else if (msgtype == MESSAGE) {
        Msg_chat_message * msg = va_arg(vl, Msg_chat_message *);
        i = msgrcv(own_id, msg, sizeof(int) + 6  + STR_BUF_SIZE + 2*USER_NAME_MAX_LENGTH, msgtype, IPC_NOWAIT);
    }
	else if (msgtype == USERS || msgtype == ROOMS || msgtype == ROOM_USERS) {
		Msg_request_response * msg = va_arg(vl, Msg_request_response *);
		i = msgrcv(own_id, msg, sizeof(char) * MAX_SERVERS_NUMBER * MAX_USERS_NUMBER * USER_NAME_MAX_LENGTH, msgtype, IPC_NOWAIT);
	}
	else i = FAIL;
	va_end(vl);
	return i; 
